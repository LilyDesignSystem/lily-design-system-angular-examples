...
//
// Selector note: several of this app's headless components wrap a
// native element inside a custom Angular host tag (see the file header
// comment in src/app/views/book-an-appointment.ts), so wrapped text
// inputs are located by `getByLabel` (which matches `aria-label`, not
// only a `<label for>`) rather than by id -- the id set on those
// components' consumer-facing tag lands on the host, not the inner
// native element the browser actually exposes to Playwright's fill().
...
test.describe("book an appointment: happy path", () => {
  test("reason -> date and time -> your details -> check answers -> confirmation", async ({ page }) => {
    ...
    await page.getByLabel("Preferred date", { exact: true }).fill("2027-01-15");
    await page.getByLabel("Preferred time of day").selectOption("morning");
    await page.getByRole("button", { name: "Continue" }).click();
...
test.describe("book an appointment: per-step validation", () => {
  ...
  test("date and time step rejects a past date", async ({ page }) => {
    ...
    await page.getByLabel("Preferred date", { exact: true }).fill("2020-01-01");
    await page.getByLabel("Preferred time of day").selectOption("morning");
    await page.getByRole("button", { name: "Continue" }).click();
