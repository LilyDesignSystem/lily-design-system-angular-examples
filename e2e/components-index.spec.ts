...
import { components } from "../src/app/components-data";
import { suffixPatternOf } from "../src/app/suffix-pattern";

// Plan P6-T5: the /components search's category + suffix-pattern
// filters, layered on top of the existing free-text search. Ported
// from lily-design-system-svelte-sveltekit-examples (e2e/components-index.spec.ts).
// Expected counts are computed here from the same canonical data the
// page itself reads (src/app/components-data.ts, src/app/suffix-pattern.ts),
// so a future catalog change can't silently drift this test out of
