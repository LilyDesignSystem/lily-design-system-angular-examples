...
// BreadcrumbListItem is an attribute-selector component (`li[lily-breadcrumb-list-item]`): its host is the
// native <li> the consumer writes, not a wrapper element, so tests mount it via a
// small host template rather than `TestBed.createComponent(BreadcrumbListItem)` directly (the
// latter has no <li> to attach to and falls back to a bare <div>).
@Component({
  standalone: true,
  imports: [BreadcrumbListItem],
  template: `<li lily-breadcrumb-list-item [className]="className"></li>`,
})
class TestHost {
  className = "";
}
